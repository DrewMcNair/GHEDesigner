...
# Manually define the font ID if GLUT failed to load it
if "GLUT_BITMAP_HELVETICA_18" not in globals():
    GLUT_BITMAP_HELVETICA_18 = ctypes.c_void_p(8)

from time import sleep

from glfw import create_window as glfw_create_widow
from glfw import get_window_size as glfw_get_window_size
...
from glfw import set_window_size_callback as glfw_set_window_size_callback
from glfw import swap_buffers as glfw_swap_buffers
...
class gl2D:
    ...
    def glStartDragging(
        self, dragCallback, dragList, dragMaxDist, handlesize=0.05, handlewidth=3, handlecolor=[1, 1, 1]
    ):
    ...
    def glDraggingMouseMove(self, x, y, leftButtonDown):
        if self.glDraggingActive is False:
            return
    ...
    def glHandleMouseEvents(self, event):
        ...
        if type == QEvent.MouseMove:  # to read the mouse location
            if self.glMouseTextBox is not None:
                self.glMouseTextBox.setText(f"{x:.2f}" + f",  {y:.2f}")
            ...
            if self.glDraggingActive:
                self.glDraggingMouseMove(x, y, leftButtonDown)
            ...
            if self.glDraggingActive:
                self.glDraggingMouseButtonRelease(x, y)
    ...
    def glStartAnimation(
        self, drawfunc, nframes, delaytime=0, repeat=False, reverse=False, reset=True, RestartDraggingCallback=None
    ):
        if self.glAnimationIsRunning is True:
            return  # don't want multiple copies
    ...
    def glStopAnimation(self):
        if self.glAnimationCallback is None:
            return
    ...
    def glAnimate(self):
        ...
        while self.glAnimationCurrentFrame != theEnd:
            ...
            self.glAnimationCallback(
                self.glAnimationCurrentFrame, self.glAnimationNFrames
